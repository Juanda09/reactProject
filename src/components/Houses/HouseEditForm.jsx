...
export default function HouseEditForm() {
    ...
    const [updateHouse] = useUpdateHouseByCodigoMutation();
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!params.codigo) {
            console.error('Código de casa no definido');
            return;
        }
        ...
        try {
            ...
            if (file) {
                ...
                uploadImage({ codigo: params.codigo, file: formData });
            }
    ...
    const { data: house, isLoading, isError, error } = useGetHouseByCodigoQuery(params.codigo);
    if (isLoading) return (
    ...
    // Pasa defaultState y defaultCity al componente HouseForm
    return (
        <HouseForm
            handleSubmit={handleSubmit}
            handleChangeImage={handleChangeImage}
            house={house}
            defaultState={house.state}
